import torch
import numpy as np
from spade.model_bros import SpadeDataset
...
if __name__ == "__main__":
    ...
    # dataset = SpadeDataset(tokenizer, config_bert, "sample_data/devop.jsonl")
    dataset = SpadeDataset(tokenizer, config_bert, "data/vietnamese_invoice_GTGT/dev_invoice_vn.jsonl")
    ...
    try:
        # sd = torch.load("checkpoint-jpcard/best_score.pt")
        # sd = torch.load("checkpoint-eng_card/best_score.pt")
        # sd = torch.load("spade-weights/model-00900.pt")
        # sd = torch.load("./spade-weights/model-00900.pt")
        # sd = torch.load("./checkpoint-vnbill/model-least-loss.pt")
        # sd = torch.load("./checkpoint-vnbill-large/best_score.pt")
        # sd = torch.load("./checkpoint-vnbill/model-00980.pt")
        sd = torch.load("./checkpoint-bros-vninvoice-2/best_score_parse.pt")
        # sd = torch.load("./checkpoint-bros-eng_card/best_score_parse.pt")
        # sd = torch.load("./checkpoint-bros-jpcard/best_score_parse.pt")
        ...
        for i in range(len(dataset)):
            ...
            pr=post_process_v2(tokenizer, rel_s_pr, rel_g_pr, batch_parse, dataset.raw[i]["fields"])      
            gt=post_process_v2(tokenizer, test_batch.labels[0][0], test_batch.labels[0][1], batch_parse, dataset.raw[i]["fields"])
            if i==0:
            ...
            tokenizer, test_batch.labels[0][0], test_batch.labels[0][1], test_batch, dataset.fields
            )
        ...
        print("Validation_f1_parse_mean: ", sum_score_parse / len(dataset))
