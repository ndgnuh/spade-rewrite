...
class RelationTagger(nn.Module):
    ...
    def forward(self, enc):
        ...
        score_0 = torch.matmul(enc_head,
                               self.W_label_0(enc_tail).transpose(1, 2))
        score_1 = torch.matmul(enc_head,
                               self.W_label_1(enc_tail).transpose(1, 2))
...
def parse_input(
    ...
    token_boxes = []
    actual_bboxes = [
    ]  # we use an extra b because actual_boxes is already used
    token_actual_boxes = []
...
class BrosSpade(nn.Module):
    ...
    def forward(self, batch):
        ...
        # print(self.dropout(last_hidden_state).shape)
