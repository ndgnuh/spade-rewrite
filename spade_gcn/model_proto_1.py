...
from spade.proto_1 import modeling_proto
...
class ProtoSpade(nn.Module):
    def __init__(self, config_bert, fields, **kwargs):
        super().__init__()
        self.n_classes = len(fields)
        self.backbone = modeling_proto.TransfoXLModel(config_bert)
        self.config_bert = config_bert = self.backbone.config

        self.dropout = nn.Dropout(0.1)

        self.rel_s = RelationTagger(
            hidden_size=config_bert.d_embed,
