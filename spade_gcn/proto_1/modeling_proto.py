...
class RelPartialLearnableMultiHeadAttn(nn.Module):
    def __init__(
        ...
        # For bounding box
        self.box_query = nn.Linear(64, self.n_head * self.d_head, bias=False)
    ...
    def forward(self, w, r, bbox_emb=None, attn_mask=None, mems=None, head_mask=None, output_attentions=False):
        qlen, rlen, bsz = w.size(0), r.size(0), w.size(1)
        
        if self.pre_lnorm:
            box_query = self.box_query(self.layer_norm(bbox_emb))
        else:
            box_query = self.box_query(bbox_emb)
        box_query = box_query.view(qlen, qlen, bsz, self.n_head, self.d_head)
        ...
        # Attention with bbox
        EF = torch.einsum('qqbnd,kbnd->qkbn', (box_query, w_head_k)) # qlen x klen x bsz x n_head
...
class RelPartialLearnableDecoderLayer(nn.Module):
    ...
    def forward(self, dec_inp, r, bbox_emb=None, dec_attn_mask=None, mems=None, head_mask=None, output_attentions=False):
...
class ProtoPositionEmbeddings(nn.Module):
    def __init__(self, n_head, d_model):
        super().__init__()
        self.dim_bbox_sinusoid_emb = d_model // 4
        self.dim_bbox_projection = d_model // n_head
        self.bbox_sinusoid_emb = PositionalEmbedding2D(
            self.dim_bbox_sinusoid_emb, dim_bbox=8
        )
...
class ProtoEmbeddings(nn.Module):
    def __init__(self, n_token, n_head, d_embed, d_proj, cutoffs, div_val=1, sample_softmax=False, bbox_unit=100.0):
        super().__init__()
        self.bbox_unit = bbox_unit
        self.word_embeddings = AdaptiveEmbedding(n_token=n_token,
                                                 d_embed=d_embed,
                                                 d_proj=d_proj,
                                                 cutoffs=cutoffs,
                                                 div_val=div_val,
                                                 sample_softmax=sample_softmax)
        self.position_embeddings = ProtoPositionEmbeddings(n_head, d_embed)
        # self.box_proj = nn.Parameter(torch.tensor(d_proj, n_token, mo
        
        
    def forward(self, input_ids=None, position_ids=None, bboxes=None):
        # batch * seq * d_model
        word_embeddings = self.word_embeddings(input_ids)

        # batch * seq * seq * 64
        bboxes = bboxes * self.bbox_unit
        bbox_embeddings = self.position_embeddings(bboxes)
        bbox_embeddings = bbox_embeddings.transpose(1, 2).transpose(0, 1)
        return word_embeddings, bbox_embeddings
...
class TransfoXLModel(TransfoXLPreTrainedModel):
    def __init__(self, config):
        ...
        self.embeddings = ProtoEmbeddings(
            config.vocab_size, config.n_head, config.d_embed, config.d_model, config.cutoffs, div_val=config.div_val,
        )
    ...
    def init_boxmems(self, bsz):
        if self.mem_len > 0:
            mems = []
            param = next(self.parameters())
            for i in range(self.n_layer):
                empty = torch.zeros(self.mem_len, self.mem_len, bsz, self.config.d_model, dtype=param.dtype, device=param.device)
                mems.append(empty)
    ...
    def forward(
        ...
        word_emb, bbox_emb = self.embeddings(input_ids, bboxes=bboxes)
