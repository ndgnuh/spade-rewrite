...
def partially_from_pretrained(config, model_name, **kwargs):
    ...
    for (k, v) in model.named_parameters():
        if k not in pretrain_sd:
            print(f"{k} layer not found")
            continue
        if pretrain_sd[k].data.shape == v.shape:
            v.data = pretrain_sd[k].data
        else:
            print(f"{k} layer shape mismatch: {v.shape} vs {pretrain_sd[k].data.shape}")

    return model


def partially_load_state_dict(model, pretrain):
    pretrain_sd = pretrain.state_dict()
    for (k, v) in model.named_parameters():
        if k not in pretrain_sd:
            continue
...
class PerceiverSpade(nn.Module):
    def __init__(self, config, fields, **kwargs):
        super().__init__()
        self.n_classes = len(fields)

        # Initialize perceiver model
        input_preprocessor = modeling_perceiver.PerceiverTextPreprocessor(config)
        self.output_num_channels = config.d_model
        decoder = modeling_perceiver.PerceiverBasicDecoder(
            config,
            output_num_channels=self.output_num_channels,
            num_channels=config.d_latents,
            trainable_position_encoding_kwargs=dict(
                num_channels=config.d_latents, index_dims=config.max_position_embeddings
            ),
            use_query_residual=True,
            final_project=True,
        )
        self.bridge = nn.Linear(config.num_latents, config.max_position_embeddings)

        self.backbone = modeling_perceiver.PerceiverModel(
            config,
            input_preprocessor=input_preprocessor,
            decoder=decoder,
        )
        self.backbone = partially_load_state_dict(
            self.backbone,
            AutoModel.from_pretrained("deepmind/language-perceiver"),
        )
        self.config = config
    ...
    def custom_forward(self, input_ids, attention_mask):
        return self.backbone(inputs=input_ids, attention_mask=attention_mask)
