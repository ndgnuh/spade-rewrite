...
class SpadeInputEmbeddings(nn.Module):
    ...
    def __init__(self, cfg, n_dist_unit=1000, n_char_unit=1000, input_embedding_components=None):
        super().__init__()
...
class Spade(nn.Module):
    def __init__(self, bert_model, fields, n_dist_unit=1000, max_position_embeddings=512):
        super().__init__()
        self.fields = fields
        if isinstance(bert_model, str):
            self.bert = AutoModel.from_pretrained(
                bert_model, max_position_embeddings=max_position_embeddings)
        else:
            self.bert = bert_model
        self.config = cfg = self.bert.config
        ...
        self.w_segment = nn.Sequential(
            nn.Dropout(0.1),
            nn.Linear(768, 768),
            nn.Dropout(0.1),
            nn.Linear(768, 768))
        self.threshold = nn.Threshold(0.7, 0.0)
    ...
    def forward(self, input_ids, position_ids, original_length, part_indices):
        encoded = torch.zeros(part_indices[-1].stop, self.config.hidden_size)
        # combine chunks
        x, y = position_ids
        for (i, slice_) in enumerate(part_indices):
            ...
            encoded_i = encoded_i.contiguous()
            # print(encoded_i.shape, encoded[:, slice_, :].shape, (slice_))
            encoded_i = self.w_segment(encoded_i)
            encoded[slice_, :] += encoded_i.squeeze(0)

        # generate graph
        encoded = encoded[:(original_length + len(self.fields)), :]
        score = torch.cat([
