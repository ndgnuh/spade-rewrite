...
def parse_graph(score, texts, fields, strict=True):
    ...
    while not queue.empty():
        ...
        # continue
        if strict:
            assert i in node_types, f"can't find label for {texts[i]} --> {texts[j]}, the labels:\n" \
                + pformat([(texts[k], fields[v])
    ...
    while not queue.empty():
        ...
        if strict:
            assert len(
                the_group) < 2, f"{texts[i]} can be found in both {pformat([[texts[i] for i in g] for g in the_group])}"
        if len(the_group) == 0:
            groups.append([i, j])
        else:
    ...
    for (inode, ifield) in node_types.items():
        ...
        if strict:
            assert len(
                the_group) < 2, f"{texts[inode]} can be found in both {pformat([[texts[i] for i in g] for g in the_group])}"
        if len(the_group) == 0:
            groups.append([inode])
